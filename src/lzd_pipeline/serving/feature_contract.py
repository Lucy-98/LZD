"""Hop dong dac trung cua model uplift.

Active serving uses the immutable 30F handoff in
``models/uplift_voucher_30f``. The parser remains backward-compatible with
contracts generated by the optional in-repo training flow.

★ MODULE NAY PHUC VU NHIEU DANG HOP DONG
--------------------------------------------------------------------------
    76 cot   model tu notebook (artifact bundled) — mo ta ngay duoi day
    83 cot   model do `train.py` huan luyen theo feature spec hien tai
...
DEFAULT_PATH = (
    ...
    / "models" / "uplift_voucher_30f" / "feature_contract.json"
)
...
def _validate(c: ModelFeatureContract, raw: Mapping[str, Any]) -> None:
    ...
    division = raw.get("chia_viec")
    if division and len(c.redis_columns) != int(division["DE_tinh_va_luu_redis"]):
        raise ContractError(
            f"so cot redis lech: khai bao {raw['chia_viec']['DE_tinh_va_luu_redis']}, "
            f"dem duoc {len(c.redis_columns)}"
        )
    if "so_dac_trung_dien_mac_dinh" in raw and len(c.default_columns) != int(raw["so_dac_trung_dien_mac_dinh"]):
        raise ContractError(
            f"so cot mac dinh lech: khai bao {raw['so_dac_trung_dien_mac_dinh']}, "
            f"dem duoc {len(c.default_columns)}"
        )
    declared = set(division["AI_service_tu_tinh"]) if division else set()
    ...
    # luyen an contract doc thang tu store va KHONG dung cot fe_* nao, nen hop
    ...
    slots = tuple(sorted(
        (
            FeatureSlot(
                ...
                # The 30F handoff declares that all original features come
                # from the feature store. Legacy contracts spell this out via
                # `nguon_gia_tri`; absence means the same thing.
                from_redis=f.get("nguon_gia_tri", SOURCE_REDIS) == SOURCE_REDIS,
